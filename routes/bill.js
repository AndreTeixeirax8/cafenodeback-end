  ...
  connection.query(query, [
    ...
    if(!err){
        ejs.renderFile(path.join(__dirname,'',"report.ejs"),{
            ...
            contactNumber:orderDetails.contactNumber,paymentMethod:orderDetails.paymentMethod,
  ...
  connection.query(query,[id],(err,results)=>{
    if(!err){
      if(results.affectedRows == 0 ){
        return res.status(404).json({message:"Relatorio não existe"})
      }
      return res.status(200).json({message:"Relatorio apagado com sucesso"})
    }
    else{
      return res.status(500).json(err);
    }
